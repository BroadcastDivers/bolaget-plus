...
export function injectRatingContainer() {
  ...
  ratingContainer.innerHTML = '<h3 style="color: #856404; text-align: center;">Bolaget+</h3>'
...
export function setRating(
  ...
  strongElement.textContent = `${translations.rating}:`;

  const ratingText = document.createTextNode(` (${rating.rating.toString()})`);

  ratingElement.appendChild(strongElement);
...
export function setUncertain(productType: ProductType, link: null | string) {
  ...
  const ratingElement = document.createElement('div')
  ratingElement.style.cssText = 'display: flex; align-items: center; gap: 5px;'
  ratingElement.style.color = '#856404';
  ratingElement.style.textAlign = 'center';
  ratingElement.innerText = translations.uncertainMatch
...
export function showLoadingSpinner() {
  ...
  spinnerContainer.style.cssText = 'display: flex; justify-content: center; align-items: center; height: 50px; gap: 10px;';
  
  const spinner = document.createElement('div');
  spinner.className = 'spinner';
  spinner.style.cssText = 'border: 4px solid #f3f3f3; border-top: 4px solid #095741; border-radius: 50%; width: 30px; height: 30px; animation: spin 2s linear infinite;';
  
  const loadingText = document.createElement('span');
  loadingText.style.cssText = 'font-size: 14px; color: #856404;';
  loadingText.textContent = `${translations.loading}..`;
  
  spinnerContainer.appendChild(spinner);
  spinnerContainer.appendChild(loadingText);
  
  ratingContainer.appendChild(spinnerContainer);
}

function generateCapSvg(rating: number): HTMLDivElement {
  const maxCaps = 5
  const capSvg = (fill: string) => `
  <svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="28px" height="28px" viewBox="0 0 50 50" style="shape-rendering:geometricPrecision; text-rendering:geometricPrecision; image-rendering:optimizeQuality; fill-rule:evenodd; clip-rule:evenodd" xmlns:xlink="http://www.w3.org/1999/xlink">
  ...
  for (let i = 0; i < maxCaps; i++) {
    if (rating >= i + 1) {
      ...
      capsHtml += capSvg('#e4e5e9')
    }
  }

  const div = document.createElement('div');
  div.style.display = 'flex';
  div.innerHTML = capsHtml;
  return div;
}

function generateStarsSvg(rating: number): HTMLDivElement {
  const maxStars = 5
